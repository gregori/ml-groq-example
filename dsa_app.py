...
def dsa_plot_candlestick(hist, ticker):
    ...
    fig = go.Figure(
        ...
        data=[
            go.Candlestick(
                x=hist["Date"],  # Define as datas no eixo X
        ...
        ]  # Define os preços de fechamento
    )
...
def dsa_plot_volume(hist, ticker):
    ...
    fig = px.bar(
        hist, x="Date", y="Volume", title=f"{ticker} Trading Volume (Últimos 6 Meses)"
...
if st.sidebar.button("Suporte"):
    st.sidebar.write(
        "No caso de dúvidas envie e-mail para: suporte@datascienceacademy.com.br"
    )
